...
Nesta seção serão apresentados os materiais utilizados para realizar as simulações propostas, tanto em questões de \textit{software}, como de \textit{hardware}.
...
Podemos observar na Figura~\ref{graph1} que o subalgoritmo \textit{insertionsort} apresentou os piores tempos médios de execução em relação aos outros algoritmos. Para o tamanho máximo analisado, com 200000 elementos, o algoritmo chegou a utilizar, em média, $60$ segundos em sua execução.

Como os tempos médios do algoritmo \textit{insertionsort} foram bastante elevados, não foi possível identificar os tempos de execução médios dos outros subalgoritmos utilizando a Figura~\ref{graph1}. Para fins de comparação entre os métodos, todos os algoritmos, com exceção do \textit{insertionsort} devido aos seus altos tempos médios de execução, foram comparados.

Podemos observar na Figura~\ref{graph2} que os tempos médios dos algoritmos \textit{mergesort}, \textit{heapsort} e \textit{quicksort} não ultrapassaram $0,09$ segundos. O \textit{quicksort} foi o subalgoritmo com melhores tempos médios de execução, não ultrapassando a barreira de $0,04$ segundos para todos os tamanhos de \textit{arrays} analisados.
...
Podemos concluir que a escolha do subalgoritmo utilizado no interior do algoritmo de ordenação \textit{bucketsort} gera grande impacto no resultado final. Quanto melhor for o desempenho do subalgoritmo de ordenação escolhido para a ordenação dos baldes, mais eficiente o \textit{bucketsort} será.
